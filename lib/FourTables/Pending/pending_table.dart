...
class PendingTable extends StatefulWidget {
  ...
  const PendingTable({Key? key, this.initialValue = false, this.onChanged})
    : super(key: key);

  @override
  PendingTableState createState() => PendingTableState();
}

class PendingTableState extends State<PendingTable> {
  ...
  void initState() {
    ...
    tableRows.addAll([
      TableRowData(
        ...
        testDate: '2025-08-20',
        clientDeliveryStatus: 'لم يتم التسليم',
  ...
  // Public method to update visible columns from external widgets
  ...
  bool _isDeliveryLate(String deliveryDate) {
    try {
      DateTime expectedDate = DateTime.parse(deliveryDate);
      DateTime today = DateTime.now();
      // Remove time component for comparison
      DateTime todayDate = DateTime(today.year, today.month, today.day);
      DateTime expectedDateOnly = DateTime(
        expectedDate.year,
        expectedDate.month,
        expectedDate.day,
      );

      return expectedDateOnly.isBefore(todayDate);
    } catch (e) {
      return false;
    }
  }
