...
class Shortcomings extends StatelessWidget {
  final bool initialValue;
  final Function(bool?)? onChanged;
  final ScrollController _verticalController = ScrollController();
  final ScrollController _horizontalController = ScrollController();
  final controller2 = Get.find<ShortcomingsController>();

  Shortcomings({Key? key, this.initialValue = false, this.onChanged})
    : super(key: key);

  @override
  Widget build(BuildContext context) {
    final controller = Get.put(ShortcomingsController());
  ...
  Widget _buildSortableHeaderCell(
    String title,
    ShortcomingsController controller,
  ) {
    return Container(
      width: _getColumnWidth(title),
      height: 60,
